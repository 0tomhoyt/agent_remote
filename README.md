...
日常使用优先记住这一组命令。下面以 HTTP relay server 为例：

```bash
# Relay 机器，例如 Windows PC：启动 relay server
...
# 编译机：查看日志
python -m agent_remote.cli --config examples/build-host.http.config.json logs \
  <job_id> \
  --target exec-a \
  --tail 200

# 编译机：拉取结果
python -m agent_remote.cli --config examples/build-host.http.config.json fetch \
  <job_id> \
  --target exec-a \
  --out ./results/<job_id>
```
...
| Relay server | 是 | `relay-server` | `host`、`port`、`storage_root` |
...
HTTP relay 模式下，应用配置里写中转机 IP/端口：
...
共享目录模式下，应用配置里不写 relay IP，而是写挂载后的路径：
...
两种模式都属于 relay-first 架构。HTTP relay server 更符合“Windows PC 作为网络中转服务”的直觉；共享目录模式更简单，适合已有挂载能力的环境。

## 主模式：HTTP Relay Server 三机部署

这是推荐部署方式，尤其适合你的 Windows 个人 PC 做网络中转。
...
执行机只需要主动访问 relay，不需要对编译机开放端口。这对企业内网和隔离网络更友好，也是 relay 模式优先于 SSH direct 的主要原因。长期推荐用 HTTP relay server；共享目录模式更适合已有挂载能力的环境。
